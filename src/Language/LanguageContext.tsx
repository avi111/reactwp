...
const extractTranslation = (data: TranslationData): Record<string, Record<Language, string>> => {
    ...
    for (const key in data) {
        ...
        if (page["wpcf-paragraph"] && page["wpcf-paragraph"].includes("1")) {
            translations[key] = {
                english: page["wpcf-paragraph-english"] ? page["wpcf-paragraph-english"].join('') : "",
                hebrew: page["wpcf-paragraph-hebrew"] ? page["wpcf-paragraph-hebrew"].join('') : "",
                russian: page["wpcf-paragraph-russian"] ? page["wpcf-paragraph-russian"].join('') : ""
            };
        } else {
            translations[key] = {
...
export const LanguageProvider: FC<LanguageProviderProps> = ({children}) => {
    ...
    const [language, setLanguage] = useState<Language>(initialLanguage || 'english'); // Default language is English
    ...
    return (
        <LanguageContext.Provider value={{translations, language, changeLanguage, translate}}>
            {children}
